...
class HCSACScaler(nn.Module):
    def __init__(self, value):
        super(HCSACScaler, self).__init__()
...
class HCSACActor(nn.Module):
    def __init__(self, num_tasks, state_dims, action_dims, num_layers=2, hidden_dims=256):
        ...
        self.log_sigma_scale = HCSACScaler(1.0)
...
class HCSACCritic(nn.Module):
    ...
    def forward(self, task, state, action, repeat=None):
        ...
        latent = self.latent(torch.cat([embedding, action], dim=-1))
...
class HCSACAgent(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.log_alpha = HCSACScaler(0.0)
        self.alpha_optimizer = Adam(self.log_alpha.parameters(), lr=actor_lr)

        self.actor = HCSACActor(num_tasks, state_dims, action_dims, num_layers, hidden_dims)
        self.actor_optimizer = Adam(self.actor.parameters(), lr=actor_lr)

        self.critic_1 = HCSACCritic(num_tasks, state_dims, action_dims, num_layers, hidden_dims)
        self.critic_2 = HCSACCritic(num_tasks, state_dims, action_dims, num_layers, hidden_dims)
    ...
    def compute_value(self, task, state, action):
        with torch.no_grad():
            value_1 = self.critic_1(task.unsqueeze(0), state.unsqueeze(0), action.unsqueeze(0))
            value_2 = self.critic_2(task.unsqueeze(0), state.unsqueeze(0), action.unsqueeze(0))
    ...
    def train_batch(self, batch, warmup=False):
        ...
        pred_action, log_pi = self.actor(task, state)
        alpha = self.log_alpha().exp()
        alpha_loss = -(self.log_alpha() * (log_pi + self.target_entropy).detach()).mean()

        if warmup:
            log_prob = self.actor.log_prob(task, state, action)
            actor_loss = (alpha * log_pi - log_prob).mean()
        else:
            pred_value = torch.min(self.critic_1(task, state, pred_action), self.critic_2(task, state, pred_action))
        ...
        pred_next_action, next_log_pi = self.actor(task, next_state)
        target_value = torch.min(self.target_critic_1(task, next_state, pred_next_action), self.target_critic_2(task, next_state, pred_next_action))
        ...
        cql_current_action, cql_current_log_pi = self.actor(task, state, repeat=self.num_samples)
        cql_current_action, cql_current_log_pi = cql_current_action.detach(), cql_current_log_pi.detach()
        cql_next_action, cql_next_log_pi = self.actor(task, next_state, repeat=self.num_samples)
        cql_next_action, cql_next_log_pi = cql_next_action.detach(), cql_next_log_pi.detach()

        cql_random_value_1 = self.critic_1(task, state, cql_random_action, repeat=self.num_samples)
        cql_random_value_2 = self.critic_2(task, state, cql_random_action, repeat=self.num_samples)
        cql_current_value_1 = self.critic_1(task, state, cql_current_action, repeat=self.num_samples)
        cql_current_value_2 = self.critic_2(task, state, cql_current_action, repeat=self.num_samples)
        cql_next_value_1 = self.target_critic_1(task, next_state, cql_next_action, repeat=self.num_samples)
        cql_next_value_2 = self.target_critic_2(task, next_state, cql_next_action, repeat=self.num_samples)

...
class MTCSACScaler(nn.Module):
    def __init__(self, value):
        super(MTCSACScaler, self).__init__()
...
class MTCSACActor(nn.Module):
    def __init__(self, num_tasks, state_dims, action_dims, num_layers=2, hidden_dims=256):
        ...
        self.log_sigma_scale = MTCSACScaler(1.0)
        self.log_sigma_bias = MTCSACScaler(-1.0)
...
class MTCSACCritic(nn.Module):
    def __init__(self, num_tasks, state_dims, action_dims, num_layers=2, hidden_dims=256):
        super(MTCSACCritic, self).__init__()
        self.embedding = nn.Embedding(num_tasks, state_dims)
...
class MTCSACAgent(object):
    """Multi-Task Conservative Soft Actor-Critic"""
    def __init__(
        self,
        num_tasks,
        state_dims,
        ...
        **kwargs
        ...
        self.log_alpha = MTCSACScaler(0.0)
        self.alpha_optimizer = Adam(self.log_alpha.parameters(), lr=actor_lr)

        self.actor = MTCSACActor(num_tasks, state_dims, action_dims, num_layers, hidden_dims)
        self.actor_optimizer = Adam(self.actor.parameters(), lr=actor_lr)

        self.critic_1 = MTCSACCritic(num_tasks, state_dims, action_dims, num_layers, hidden_dims)
        self.critic_2 = MTCSACCritic(num_tasks, state_dims, action_dims, num_layers, hidden_dims)

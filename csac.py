...
from agent.csac import CSACAgent
from utils.torch import convert_batch_to_tensor
from utils.dataset import CSACDataset, csac_collate
...
@hydra.main(version_base=None, config_path='./config', config_name='csac')
def main(config):
    ...
    for epoch in range(1, config.strategy.num_epochs + 1):
        ...
        if epoch % config.strategy.eval_interval == 0:
            ...
            for key, value in feedback.items():
                report.append(f'{key}: {np.mean(value):7.2f}')
            report = ' | '.join(report)
            print(f'Evaluate | Epoch: {epoch:4d} | Task: {task:11} | {report} |')
            if np.mean(feedback['Average Return']) > best:
                best = np.mean(feedback['Average Return'])
